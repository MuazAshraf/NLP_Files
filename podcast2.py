...
"""
        ...
        try:
            ...
            if "API key not valid" in str(e):
                ...
                raise ValueError("Rate limit exceeded for the API key. Please try again later or provide your own Gemini API key.")
            else:
                raise ValueError(f"Failed to generate podcast script: {e}")
    ...
    async def generate_podcast(self, input_text: str, language: str, speaker1: str, speaker2: str, api_key: str) -> str:
        ...
        print(f"Successfully generated podcast script in {(time.time() - start_time):.2f} seconds!")
        ...
        print(f"Successfully generated podcast audio files in {(time.time() - start_time):.2f} seconds!")
...
async def generate_podcast():
    try:
        ...
        data = request.json
        # Validate input - ensure either text or file is provided, not both
        input_text = data.get('input_text')
        input_file = data.get('input_file')
        
        if not input_text and not input_file:
            return jsonify({'error': 'Either input_text or input_file must be provided'}), 400
        if input_text and input_file:
            return jsonify({'error': 'Cannot provide both input_text and input_file'}), 400

        language = data.get('language', 'Auto Detect')
        speaker1 = data.get('speaker1', 'Andrew - English (United States)')
        speaker2 = data.get('speaker2', 'Ava - English (United States)')
    ...
    # Handle file input if provided
        if input_file:
            ...
            with open(temp_file_path, 'wb') as f:
                f.write(input_file)
            input_text = await TextExtractor.extract_text(temp_file_path)
            os.remove(temp_file_path)

        # Generate podcast
        podcast_generator = PodcastGenerator()
        podcast_file = await podcast_generator.generate_podcast(
            input_text, language, speaker1, speaker2, GEMINI_API_KEY
        )
