...
pydantic
requests
Creating .env file to store all credentials
...
LLAMA_API_KEY = "xxxx"
Create a python file 'scraper.py', and import all libraries
...
import tempfile
import requests
from typing import List, Optional, Dict, Any, Type, Union, get_type_hints
...
# Initialize OpenAI client wrapped with Langsmith for tracing
...
We need to define the structure of the data we want to extract. We'll use Pydantic models for this. Here's an example for a restaurant menu:
...
1.2 Creating a Filtered Model
The create_filtered_model function is responsible for dynamically creating a Pydantic model based on the data points we're looking to extract. This allows us to focus only on the fields that haven't been filled yet, making our extraction process more efficient.
...
2. Extracting Data from Content
The extract_data_from_content function is the core of our LLM-based extraction process. It uses the Instructor-enhanced OpenAI client to extract structured data from the scraped content.
...
def extract_data_from_content(content, data_points, links_scraped, url):
    ...
    return result.json()
Meanwhile, We also built a filter_empty_fields function, which is crucial for cleaning up the extracted data. It removes any fields that are empty or contain default values, ensuring that we only update our data structure with meaningful information. Because sometimes LLM can return None or null results; we don't really want to update those data
...
Recursively filters out empty or default values from dictionaries and lists.

Preserves the type information for each field.

Returns a cleaned dictionary with only non-empty fields.

By combining these functions, we create a system for extracting structured data from unstructured text:

We start by creating a filtered model that focuses only on the data points we still need to extract.

We then use this model to guide the LLM in extracting relevant information from the scraped content.

Finally, we clean the extracted data, removing any empty or default values, before updating our main data structure.





Step 2: Building core scraper agent tools
1. Llama parse tool
...
Step 4: Define agent workflow stage
...
1 - Code breakdown - Agent template: Agentic web scraper · AI Builder Club