...
def save_report(content, output_format, filename_base):
    ...
    if output_format == "markdown":
        ...
        # Convert markdown to HTML then strip HTML tags for PDF
        html = markdown.markdown(content)
        # Simple HTML stripping - you might want a more sophisticated approach
...
async def generate_report():
    try:
        ...
        output_format = data.get("output_format", "markdown").lower()
        
        if not topic:
            return jsonify({"error": "Topic is required"}), 400
            
        if output_format not in ["markdown", "text", "pdf"]:
        ...
        # Generate filename base from topic
        filename_base = re.sub(r'[^\w\s-]', '', topic).strip().lower().replace(' ', '_')
