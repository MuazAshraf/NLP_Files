...
def main():
    # Initialize the browser with downloads folder
    browser = SimpleTextBrowser(
        downloads_folder="./downloads",
        serpapi_key="91c0ec2420475600ef130c54212d35de6b945997700fce0e2ea29a954cc7bd82"  
    )

    # Create all tools
    search_tool = SearchInformationTool(browser)
    visit_tool = VisitTool(browser)
    page_down = PageDownTool(browser)
    finder = FinderTool(browser)
    ...
    print("\n=== Visiting Python.org ===")
    result = visit_tool.forward("https://python.org")
    print(result)
    all_results.append("# Visiting Python.org\n" + result)

    # Example 2: Find text on page
    print("\n=== Finding 'Python' on the page ===")
    result = finder.forward("Python")
    print(result)
    all_results.append("\n## Finding 'Python' on the page\n" + result)

    # Example 3: Page down to see more content
    print("\n=== Scrolling down the page ===")
    result = page_down.forward()
    print(result)
    all_results.append("\n## Scrolling down the page\n" + result)

    # Example 4: Try a web search (requires SerpAPI key)
    if browser.serpapi_key:
        print("\n=== Searching for 'Python programming' ===")
        result = search_tool.forward("Python programming")
        print(result)
        all_results.append("\n## Web Search Results for 'Python programming'\n" + result)
    else:
        print("\nNote: Web search requires a SerpAPI key")
        all_results.append("\nNote: Web search requires a SerpAPI key")

    # Example 5: Check archived version of a webpage
    print("\n=== Checking archived version of Python.org from 2010 ===")
    try:
        result = archive_tool.forward("https://python.org", "20100101")
        print(result)
    ...
    try:
        result = download_tool.forward("https://www.python.org/static/community_logos/python-logo.png")
        print(result)
    ...
    with open("web_browser_results.md", "w", encoding="utf-8") as f:
        f.write("\n\n".join(all_results))
    
    print("\nResults have been saved to 'web_browser_results.md'")
